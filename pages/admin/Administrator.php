<?php
    ...
    $title = 'QUẢN LÝ QUẢN TRỊ VIÊN | '.$TUANORI->site('title');
                        ...
                        <div class="table-responsive">
                            <table id="datatable" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($TUANORI->get_list(" SELECT * FROM `users` WHERE `level` = 'admin' ORDER BY id DESC ") as $row){
                                    ?>
                                    <tr>
                                        <td>
                                            <ul>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status == 'success') {
                    Swal.fire("Thông Báo","Đã xóa thành công quản trị viên: " + username, "success");
                    setTimeout(function() {
                        location.reload();
                    }, 1000);
                } else {
                    Swal.fire("Thông Báo", "Đã xảy ra lỗi khi xoá quản trị viên: " + username, "success");
                }
            }
        });
    }
