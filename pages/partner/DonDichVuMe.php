<?php
    ...
    $title = 'ĐƠN DỊCH VỤ ĐANG LÀM | '.$TUANORI->site('title');
...
<div class="content-wrapper">
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>ĐƠN HÀNG CÀY THUÊ ĐANG LÀM</h1>
                </div>
            </div>
        </div>
    </section>
                        ...
                        <div class="table-responsive">
                            <table id="datatable1" class="table table-bordered table-striped table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $i = 0;
                                    foreach($TUANORI->get_list(" SELECT * FROM `history_dichvu` WHERE `dichvu` > 0 AND `ctv` = '".$getUser['username']."' AND `status` = '0' ORDER BY id DESC LIMIT 200") as $row){
                                    ?>
                                    <tr>
                                        <td><?=++$i;?></td>
                                        ...
                                        <td>
                                            <a aria-label="" href="/Partner/EditDonHang/<?=$row['id'];?>" style="color:white;" class="btn btn-info btn-sm btn-icon-left m-b-10" type="button">
