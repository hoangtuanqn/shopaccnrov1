            ...
            <div class="container">
                ...
                ?>
                    <div class="c-content-title-1" style="margin-top: 40px">
                        <h3 class="c-font-uppercase c-font-bold">LỊCH SỬ MUA VÀNG</h3>
                    ...
                    <table id="charge_recent" class="table table-striped table-custom-res">
                        ...
                        <tbody>
                        <?php $i = 0; foreach($TUANORI->get_list(" SELECT * FROM `history_dichvu` WHERE `username` = '".$getUser['username']."'  AND `dichvu` = '-1' ORDER BY id DESC LIMIT $from,$sotin1trang") as $row){ ?>
                            <tr>
                                <td><?=++$i;?></td>
                                <td><?=$row['tk'];?></td>
                                <td>Vũ Trụ <?=$row['server'];?></td>
                                <td><b style="color: green"><?=number_format($row['tongtien']);?>đ</b></td>
                                <td><b style="color: green"><?=number_format($row['iteam']);?> vàng</b></td>
                    ...
                    <center>
                        <div class="row">
                            <?php
                                $tong = $TUANORI->num_rows(" SELECT * FROM `history_dichvu` WHERE `username` = '".$getUser['username']."' AND `dichvu` = '-1' ");
                                if ($tong > $sotin1trang)
                                {
